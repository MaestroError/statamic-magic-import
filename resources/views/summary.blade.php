...
@section('content')
    ...
    <header class="mb-3"><h1>JSON Import - summary</h1></header>

    <div class="content">
        <script>
            window.ImportSummary = {!! json_encode($summary) !!};
            window.existingCollections = {!! json_encode($collections) !!};
        </script>

        <!-- VUE template Start -->

        <json-importer inline-template token="{{ csrf_token() }}">
            <div>
                    ...
                    <div>
                        ...
                        <div v-if="showCollectionsSection" v-for="(collection, collectionName) in summary.collections" class="card mb-3">
                            ...
                            <div class="import-type-container">
                                ...
                                <select @change="setCollectionImportType(collectionName, $event)" id="import-type">
                                    <option value="new">Create new collection</option>
                                    <option value="import">Import in collection</option>
                                </select>
                            </div>
                                    ...
                                    <div class="form-group pb-3 import-in">
                                        ...
                                        <select @change="setCollectionPair(collectionName, $event)"  class="form-control">
                                            ...
                                            @foreach($collections as $coll)
                                                <option value="{{$coll['handle']}}">{{$coll['title']}}</option>
                                            @endforeach
                                            </div>
                                        </select>
                                    </div>
